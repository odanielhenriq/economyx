<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Nova conta fixa</h2>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8 space-y-6">
            ...
            @php
                $prefill = $prefill ?? [];
                $prefillUsers = collect(old('user_ids', $prefill['user_ids'] ?? []));
                $prefillStartDate = $prefill['start_date'] ?? $prefill['transaction_date'] ?? null;
            @endphp
                ...
                <div class="p-6 bg-white rounded shadow-sm border space-y-4">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <input type="number" step="0.01" name="amount" value="{{ old('amount', $prefill['amount'] ?? '') }}"
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="total_amount"
                                value="{{ old('total_amount', $prefill['total_amount'] ?? '') }}"
                                class="mt-1 w-full rounded border-gray-300 text-sm" placeholder="0,00">
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <select name="frequency" class="mt-1 w-full rounded border-gray-300 text-sm">
                                <option value="monthly" @selected(old('frequency', $prefill['frequency'] ?? 'monthly') === 'monthly')>Mensal</option>
                                <option value="yearly" @selected(old('frequency', $prefill['frequency'] ?? '') === 'yearly')>Anual</option>
                            </select>
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Dia do mes</label>
                            <input type="number" name="day_of_month" value="{{ old('day_of_month', $prefill['day_of_month'] ?? '') }}"
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <input type="date" name="start_date" value="{{ old('start_date', $prefillStartDate) }}"
                                class="mt-1 w-full rounded border-gray-300 text-sm">
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Fim (opcional)</label>
                            <input type="date" name="end_date" value="{{ old('end_date', $prefill['end_date'] ?? '') }}"
                                class="mt-1 w-full rounded border-gray-300 text-sm">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300"
                            @checked(old('is_active', $prefill['is_active'] ?? 1))>
                    ...
                    <div>
                        ...
                        <select name="category_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected(old('category_id', $prefill['category_id'] ?? '') == $category->id)>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="type_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type->id }}" @selected(old('type_id', $prefill['type_id'] ?? '') == $type->id)>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="payment_method_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($paymentMethods as $paymentMethod)
                                <option value="{{ $paymentMethod->id }}"
                                    @selected(old('payment_method_id', $prefill['payment_method_id'] ?? '') == $paymentMethod->id)>
                                    {{ $paymentMethod->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="credit_card_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($creditCards as $card)
                                <option value="{{ $card->id }}" @selected(old('credit_card_id', $prefill['credit_card_id'] ?? '') == $card->id)>
                    ...
                    <div>
                        ...
                        <select name="user_ids[]" multiple class="mt-1 w-full rounded border-gray-300 text-sm">
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @selected($prefillUsers->contains($user->id))>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
