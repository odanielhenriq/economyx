<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-lg font-semibold text-slate-900">Nova categoria</h1>
    ...
    <div class="space-y-6">
        ...
        <form method="POST" action="{{ route('categories.store') }}" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Cor da categoria</label>
                    <div class="flex items-center gap-3">
                        <input type="color" name="color"
                            value="{{ old('color', '#16a34a') }}"
                            class="h-10 w-16 rounded-lg border border-slate-200 cursor-pointer p-1 bg-white">
                        <span class="text-sm text-slate-500">Clique para escolher uma cor</span>
                    </div>
                </div>
            </div>
