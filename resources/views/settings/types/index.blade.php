<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Tipos</h2>
            <a href="{{ route('types.create') }}" class="px-3 py-1 text-sm text-white bg-indigo-600 rounded hover:bg-indigo-700">
                + Novo tipo
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm text-left">
                            ...
                            <tbody id="types-table" class="divide-y">
                                <tr>
            ...
            fetch('/api/types', {
                ...
                .then((payload) => {
                    ...
                    tableBody.innerHTML = items.map((type) => {
                        ...
                        const colorInfo = type.color
                            ? `
                                <span class="inline-flex items-center gap-2">
                                    <span class="inline-block w-3 h-3 rounded" style="background-color: ${escapeHtml(type.color)}"></span>
                                    <span class="text-gray-600">${escapeHtml(type.color)}</span>
                                </span>
                            `
                        ...
                        return `
                            <tr>
                                <td class="px-3 py-2">${escapeHtml(type.name)}</td>
                                <td class="px-3 py-2 text-gray-500">${escapeHtml(type.slug)}</td>
                                <td class="px-3 py-2">${colorInfo}</td>
                                <td class="px-3 py-2 text-right">
                                    <div class="flex justify-end gap-2">
                                        <a href="${editUrl}" class="text-indigo-600 hover:underline">Editar</a>
                                        <form method="POST" action="${deleteUrl}" onsubmit="return confirm('Remover este tipo?');">
