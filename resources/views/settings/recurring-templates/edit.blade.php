<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Editar template recorrente</h2>
            <a href="{{ route('recurring-templates.index') }}" class="text-sm text-indigo-600 hover:underline">
                Voltar
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('recurring-templates.update', $recurringTemplate) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 bg-white rounded shadow-sm border space-y-4">
                    <div>
                        ...
                        <input type="text" name="description"
                            value="{{ old('description', $recurringTemplate->description) }}"
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <select name="frequency" class="mt-1 w-full rounded border-gray-300 text-sm">
                                ...
                                <option value="yearly" @selected(old('frequency', $recurringTemplate->frequency) === 'yearly')>Anual</option>
                            </select>
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Dia do mes</label>
                            <input type="number" name="day_of_month"
                                value="{{ old('day_of_month', $recurringTemplate->day_of_month) }}"
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <input type="date" name="start_date"
                                value="{{ old('start_date', optional($recurringTemplate->start_date)->format('Y-m-d')) }}"
                                class="mt-1 w-full rounded border-gray-300 text-sm">
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Fim (opcional)</label>
                            <input type="date" name="end_date"
                                value="{{ old('end_date', optional($recurringTemplate->end_date)->format('Y-m-d')) }}"
                                class="mt-1 w-full rounded border-gray-300 text-sm">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300"
                            @checked(old('is_active', $recurringTemplate->is_active))>
                        ...
                        <select name="category_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    @selected(old('category_id', $recurringTemplate->category_id) == $category->id)>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="type_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type->id }}"
                                    @selected(old('type_id', $recurringTemplate->type_id) == $type->id)>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="payment_method_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($paymentMethods as $paymentMethod)
                                <option value="{{ $paymentMethod->id }}"
                                    @selected(old('payment_method_id', $recurringTemplate->payment_method_id) == $paymentMethod->id)>
                                    {{ $paymentMethod->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="credit_card_id" class="mt-1 w-full rounded border-gray-300 text-sm">
                            ...
                            @foreach ($creditCards as $card)
                                <option value="{{ $card->id }}"
                                    @selected(old('credit_card_id', $recurringTemplate->credit_card_id) == $card->id)>
