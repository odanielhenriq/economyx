@props([
    'title',
    'description' => null,
])

<div {{ $attributes->merge(['class' => 'bg-white rounded-xl border border-slate-200 shadow-sm px-6 py-12 sm:py-14 text-center']) }}>
    @if (isset($icon))
        <div class="flex justify-center">{{ $icon }}</div>
    @else
        <svg class="mx-auto h-10 w-10 text-slate-300" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
        </svg>
    @endif

    <h3 class="mt-4 text-base font-semibold text-slate-900">{{ $title }}</h3>

    @if ($description)
        <p class="mt-2 text-sm text-slate-500 max-w-md mx-auto leading-relaxed">{{ $description }}</p>
    @endif

    @if (isset($actions))
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
            {{ $actions }}
        </div>
    @endif
</div>
